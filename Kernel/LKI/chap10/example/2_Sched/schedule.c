...
/*                my_scheduler는 선점형의 라운드로빈방식의 스케줄러입니다.     */
/*		  schedule.c에서는 main함수에서 정의된 함수들의                */
/*                스케줄링을 위한 내부동작을 구현했습니다.                     */
/* Date         : 2008/07/05                                                   */
/*                                                                             */
/*******************************************************************************/
...
/* global scheduler handeler를 초기화하고, parent_task를 생성한다. */
void thread_init()
{
...
// linkedlist에서 gh_sch.running_task가 가르키고 있는 task 리턴
TaskInfo task_get_runningtask(void)		
{
	return gh_sch.running_task;
}

// linkedlist에서 gh_sch.running_task가 가르키고 있는 task의 다음 task 리턴
void task_next(void)	
{
	TaskInfo temp;
