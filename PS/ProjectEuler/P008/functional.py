...
1. get 13 adjacent digits from 1000-digit number
2. calculate products, compare
3. preserve greater one
4. last preserved one is the greatest product
...
def adjacent_products(nums):
    product = reduce(lambda x, y: x * y, nums)
    return product

if __name__ == '__main__':
    # Generate adjacent number lists
    adjacent_lists = [NUMBERS[i:i+ADJACENT]
            for i in range(0, len(NUMBERS)-ADJACENT + 1)]
    # Filter adjacent lists which contains 0 (which leads product into 0)
    adjacent_lists = list(filter(lambda l: 0 not in l, adjacent_lists))
    # Calculate products
    products = list(map(adjacent_products, adjacent_lists))
    # Answer is the maximum value of products
    answer = max(products)
